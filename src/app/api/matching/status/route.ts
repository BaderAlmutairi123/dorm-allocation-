...
export async function GET() {
  try {
    ...
    // Blocks error is not critical, just log it
    if (blocksError) {
      console.warn('Failed to fetch blocks count:', blocksError.message)
    }
    ...
    return NextResponse.json({
      ...
      canRunMatching: (pendingCount || 0) > 0 && availableRoomsCount > 0,
    })
  } catch (error: any) {
